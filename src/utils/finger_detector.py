"""
Adapted from https://github.com/PySeez/middle-finger-detection
"""
import math
import cv2
...
class FingerDetector:
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                (
                    ...
                    ring_len,
                    pinky_len,
                ) = self._check_finger(hand_landmarks)
                out.append(
                    (
                        hand_landmarks,
                        finger_stat,
    ...
    def mid_finger_region(self, image, hand_landmarks):
        ...
        tip_pos = int(l[HandLandmark.MIDDLE_FINGER_TIP].x * w), int(
            l[HandLandmark.MIDDLE_FINGER_TIP].y * h
        )
        mcp_pos = int(l[HandLandmark.MIDDLE_FINGER_MCP].x * w), int(
            l[HandLandmark.MIDDLE_FINGER_MCP].y * h
        )

        return tip_pos, mcp_pos

    def _check_finger(self, hand_landmarks, threshold=0.7):
        HandLandmark = self.mp_hands.HandLandmark
        ...
        if index_len * threshold > max(mid_len, ring_len, pinky_len):
            return HandGesture.Point, mid_len, index_len, ring_len, pinky_len
        if mid_len * threshold > max(index_len, ring_len, pinky_len):
            return HandGesture.Mid, mid_len, index_len, ring_len, pinky_len
        if (index_len + mid_len) / 2 * threshold > max(ring_len, pinky_len):
            return HandGesture.Yeah, mid_len, index_len, ring_len, pinky_len

        return HandGesture.No, mid_len, index_len, ring_len, pinky_len
